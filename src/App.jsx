...
function App() {
  ...
  useEffect(() => {
    const initEmulator = async () => {
      if (emulator) return; // Evita reinicializaciones

      try {
        // 1. Importamos el motor como un módulo real de JS
        ...
        if (canvasRef.current && mGBAFunc) {
          const Module = await mGBAFunc({
            ...
            // Ruta absoluta para asegurar que Vercel encuentre el .wasm
  ...
  return (
    <div style={{ backgroundColor: '#121212', color: '#fff', minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', fontFamily: 'sans-serif' }}>
      ...
      <canvas ref={canvasRef} tabIndex="0" style={{ width: '480px', height: '320px', backgroundColor: '#000', border: '5px solid #333', imageRendering: 'pixelated' }} />
